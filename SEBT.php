...
$cocktailQ = unserialize(base64_decode($_POST["cocktailQ"]));
  ...
  if($_POST["submit5"] && $Z>0){
    ...
    if(isset($_POST["ELoss"])){$EnergyLoss=$_POST["ELoss"];}
    echo "<form action=\"SEBT.php\" target=\"ISACFrame\" method=\"post\">";
  ...
  echo '<input type="hidden" name="cocktailQ" value="', base64_encode(serialize($cocktailQ)),'">';
    ...
    echo " MeV/u.<BR>";
    echo "Percentage energy loss in the stripping foil is: <input type=\"text\" name=\"ELoss\" value=\"";
      if($EnergyLoss>0){printf("%.1f",$EnergyLoss);}
    echo "\" style=\"width:50px;\">%.";
    echo "<br><input type=\"submit\" name=\"submit5\" value=\"Recalculate\" ></form>";
    ...
    echo $thickness;
    echo " microgram/cm<sup>2</sup> to reach the equilibrium charge state.<BR>\n";
    echo "Percentage Energy Loss was input as "; echo $EnergyLoss; echo "%.<BR><BR>";
    ...
    $c=(2*($Z-$meanQ+2))/($s*$s);
    $nu=(2*($Z-$meanQ+2)*($Z-$meanQ+2))/($s*$s);
      $F=charge_fraction($meanQ,$s,$meanQ);
...
// SET MINIMUM Y and X
    $ymin[$imagenum]=0.0;
    $xmin[$imagenum]=round($A/$meanQ,2)-1.0;

// SET MAXIMUM Y and X
    $xmax[$imagenum]=round($A/$meanQ,2)+0.75;
    $ymax[$imagenum]=$F+0.1;
...
// SET MINIMUM Y and X
    $ymin[$imagenum]=0.0;
    $xmin[$imagenum]=floor($meanQ)-8;

// SET MAXIMUM Y and X
    $xmax[$imagenum]=floor($meanQ)+8;
    $ymax[$imagenum]=$F+0.1;
// THIS TREATMENT OF X WORKS FOR DEGREES BUT NOT RADIANS!!!
    if($xmax[$imagenum]-round($xmax[$imagenum]) < 0.01){$xmax[$imagenum]=round($xmax[$imagenum]);}
    if($xmax[$imagenum]-round($xmax[$imagenum]) >= 0.01){$xmax[$imagenum]=floor($xmax[$imagenum]+1);}
...
echo "Here 1.0 is equal to 100% and 0.1 would correspond to 10% of beam incident on the stripping foil exiting in that charge state.<BR>\n";
    echo "<div class=\"t2\"><b>q</b></div>\n";
    echo "<div class=\"t3\"><b>Fraction</b></div>";
    echo "<div class=\"t4\"><b>A/q</b></div>\n";
    echo "<div class=\"t5\"><b>Energy loss Adjusted A/q</b></div>\n";
    echo "<BR><BR><BR>\n";
printf("<b>%d%s:</b><BR>",$A,$Element[$Z]);
    for($i=1; $i<=$Z+2; $i++){
      $t=$c*($Z-$i+2);
     // $F=frac($nu,$t,$c);
      $F=charge_fraction($i,$s,$meanQ);
	...
	echo "<div class=\"t2\">";
      ...
      printf("%5.3f",((($beammass-$i*$emass)/$i)*(sqrt(1.0-($EnergyLoss/100)))));
      echo "</div><BR>";
      }
    }

    // Charge State fractions for each species.
   for($j=0; $j<sizeof($cocktailA); $j++){
      for($k=1; $k<$cocktailZ[$j]; $k++){
	if($cocktailA[$j]==$A && $cocktailZ[$j]==$Z){continue;}
        $thismass=getthismass($cocktailA[$j],$cocktailZ[$j]);
...
//	$F=frac($nu,$t,$c);
	...
	echo "<div class=\"t3\">";
	if($F>0.001){ printf("%5.3f",$F); }else{echo "0.000";}
	echo "</div><div class=\"t4\">";
	printf("%5.3f",($thismass-$ii*$emass)/$ii);
	echo "</div><div class=\"t5\">";
	printf("%5.3f",((($thismass-$ii*$emass)/$ii)*(sqrt(1.0-($EnergyLoss/100)))) );
	echo "</div><BR>";
